...
import { activityTypes, habits, timeBlocks, todos } from '@auto-cal/db/schema';
...
/**
 * Runs computeSchedule for the given user and persists the resulting scheduledAt
 * timestamps back to the todos table.
 */
export async function persistScheduleForUser(
  userId: string,
  db: DB,
): Promise<void> {
  // Fetch all data needed for scheduling
  const [userTimeBlocks, incompleteTodos, userHabits, userActivityTypes] =
    await Promise.all([
      ...
      db._query.habits.findMany({ where: eq(habits.userId, userId) }),
  ...
  // Build habit instances based on frequencyCount
  const habitInstances = userHabits.flatMap((h) =>
    Array.from({ length: h.frequencyCount }, (_, i) => ({
      ...h,
      instanceIndex: i,
    })),
  );
  ...
  // Persist scheduledAt for todos
  await Promise.all(
    scheduled
      .filter((item) => item.kind === 'todo')
      .map((item) => {
        const newScheduledAt =
          item.isScheduled && item.scheduledStart
